...
import { createContextKey, diag, SpanStatusCode, trace as trace$1, isSpanContextValid, TraceFlags, isValidTraceId, baggageEntryMetadataFromString, propagation, context, SpanKind, INVALID_TRACEID, ROOT_CONTEXT, DiagLogLevel } from '@opentelemetry/api';
...
class ParentBasedSampler {
    ...
    shouldSample(context, traceId, spanName, spanKind, attributes, links) {
        const parentContext = trace$1.getSpanContext(context);
   ...
   */
   inject(context, carrier, setter) {
    if (isTracingSuppressed(context)) {
      DEBUG_BUILD$1 && debug$1.log('[Tracing] Not injecting trace data for url because tracing is suppressed.');
      return;
    }
    ...
    if (!shouldPropagateTraceForUrl(url, tracePropagationTargets, this._urlMatchesTargetsMap)) {
      DEBUG_BUILD$1 &&
        debug$1.log('[Tracing] Not injecting trace data for url because it does not match tracePropagationTargets:', url);
      return;
    }
...
function shouldPropagateTraceForUrl(
  ...
  if (cachedDecision !== undefined) {
    DEBUG_BUILD$1 && !cachedDecision && debug$1.log(NOT_PROPAGATED_MESSAGE, url);
  ...
  DEBUG_BUILD$1 && !decision && debug$1.log(NOT_PROPAGATED_MESSAGE, url);
  return decision;
}
  ...
  /** @inheritDoc */
   shouldSample(
    ...
    if (method === 'OPTIONS' || method === 'HEAD') {
      DEBUG_BUILD$1 && debug$1.log(`[Tracing] Not sampling span because HTTP method is '${method}' for ${spanName}`);
    ...
    if (
      ...
      DEBUG_BUILD$1 && debug$1.log('[Tracing] Discarding root span because its trace was not chosen to be sampled.');
...
function getParentSampled(parentSpan, traceId, spanName) {
  ...
  if (isSpanContextValid(parentContext) && parentContext.traceId === traceId) {
    if (parentContext.isRemote) {
      ...
      DEBUG_BUILD$1 &&
        debug$1.log(`[Tracing] Inheriting remote parent's sampled decision for ${spanName}: ${parentSampled}`);
      return parentSampled;
    }

    const parentSampled = getSamplingDecision(parentContext);
    DEBUG_BUILD$1 && debug$1.log(`[Tracing] Inheriting parent's sampled decision for ${spanName}: ${parentSampled}`);
    return parentSampled;
  }
