...
async function listUserTables() {
  ...
  try {
    ...
    console.log(`📊 Found ${tables.length} user table(s) in the public schema:\n`);
    ...
    for (let i = 0; i < tables.length; i++) {
      ...
      try {
        ...
        try {
          const { rows: actualCountRows } = await client.query(`SELECT COUNT(*) as actual_count FROM "${schemaname}"."${tablename}"`);
