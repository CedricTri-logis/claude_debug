...
import { initializeSentry, flushSentry } from '../config/sentry.config.js';
...
async function main() {
  ...
  try {
    ...
    // Start debug session
    const debugSessionId = debugger.startSession(correlationId, {
    ...
    const debugState = debugger.dumpState(debugSessionId);
    logger.info({
...
async function demonstrateLogging(logger, debugSessionId) {
  ...
  try {
    ...
    debugger.checkpoint('logging_demo_complete', {
      logsCreated: 8
    });
  ...
  try {
    ...
    for (const error of errors) {
      try {
        ...
        // Analyze error
        const analysis = debugger.analyzeError(err, debugSessionId);
        childLogger.info({
    ...
    const correlated = debugger.correlateLogs(timestamp, 5);
...
async function demonstratePerformanceTracking(logger, debugSessionId) {
  ...
  try {
    ...
    const profile = debugger.createPerformanceProfile('demo_operation');
