  ...
  try {
    ...
    if (extensionRows.length > 0) {
      extensionRows.forEach((ext) => {
        console.log(
          `   • ${ext.extname} (v${ext.extversion}) in schema: ${ext.schema}`,
        );
      });
    } else {
    ...
    console.log(
      `   • Project ID: ${process.env.SUPABASE_URL ? process.env.SUPABASE_URL.split(".")[0].replace("https://", "") : "Not configured"}`,
    );
    console.log(
      `   • Connection Pool: Min ${process.env.DATABASE_POOL_MIN || "default"}, Max ${process.env.DATABASE_POOL_MAX || "default"}`,
    );
    ...
    try {
      ...
      console.log(
        `   • Read/Write Access: ✅ Working (test count: ${testRows[0].count})`,
      );
    } catch (error) {
    ...
    try {
      ...
      console.log(
        `   • Authentication: ✅ Connected as ${authTest[0].current_user}`,
      );
    } catch (error) {
    ...
    try {
      const { rows: catalogTest } = await client.query(
        "SELECT COUNT(*) FROM pg_tables LIMIT 1",
      );
