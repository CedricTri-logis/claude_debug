...
import { createSupabaseAdmin } from "../../shared/supabase-client.js";
import fs from "fs";
...
async function exportSchema() {
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(dbSchemaDir, "schema_documentation.md"),
      schemaDoc,
    );
    ...
    const publicTables = Object.values(tableMap).filter(
      (t) => t.schema === "public",
    );
    const dataSamples = {};
    ...
    const summary = {
      ...
      statistics: {
        ...
        auth_tables: Object.values(tableMap).filter((t) => t.schema === "auth")
          .length,
