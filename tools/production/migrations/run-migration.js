...
// Environment variables needed for fetch operations
const supabaseUrl = process.env.SUPABASE_URL;
const supabaseServiceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;

// Create validated Supabase admin client
const { admin: supabase } = createValidatedSupabaseClients({
  includeAnon: false,
});
...
async function runMigration(migrationFile, isRollback = false) {
  try {
        ...
        if (error) {
          ...
          if (error.message.includes("exec_sql")) {
            ...
            if (!response.ok) {
              ...
              console.error(
                `❌ Command failed: ${command.substring(0, 50)}...`,
              );
    ...
    console.log(
      `✅ Migration ${isRollback ? "rollback" : "execution"} completed successfully`,
    );
    return true;
  } catch (error) {
...
Available migrations:
  ...
  if (supabaseUrl.includes("supabase.co")) {
    ...
    console.log(
      "   Please ensure you have backups and have tested this migration.",
    );
