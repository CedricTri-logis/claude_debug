...
async function analyzeResults() {
  ...
  console.log(
    `${colors.bright}${colors.blue}  Parallel Task Execution Analysis${colors.reset}`,
  );
  console.log(
    `${colors.bright}${colors.blue}═══════════════════════════════════════════${colors.reset}\n`,
  );
  ...
  try {
    ...
    console.log(
      `• Total duration: ${colors.bright}${durationSeconds} seconds${colors.reset}`,
    );
    ...
    if (durationSeconds <= 15) {
      console.log(
        `${colors.green}✓ PARALLEL EXECUTION CONFIRMED${colors.reset}`,
      );
      console.log(
        `  Tasks executed in parallel (~10s expected, ${durationSeconds}s actual)`,
      );
      ...
      console.log(
        `${colors.red}✗ SEQUENTIAL EXECUTION DETECTED${colors.reset}`,
      );
      console.log(
        `  Tasks executed sequentially (~30s expected, ${durationSeconds}s actual)`,
      );
      ...
      console.log(
        `  Duration of ${durationSeconds}s doesn't clearly indicate parallel or sequential`,
      );
      console.log(
        `  This could indicate partial parallelism or other factors.`,
      );
    ...
    if (durationSeconds <= 15) {
      ...
      console.log(
        `2. Design agents to handle multiple operations when possible`,
      );
      console.log(
        `3. Consider using orchestrator patterns for complex workflows`,
      );
    }
  } catch (error) {
    console.error(
